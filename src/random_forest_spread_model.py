"""
NBA Spread Prediction - Random Forest Model
Author: Orb Analytics (Liam Chaitin)
Purpose: Backtest random forest model with detailed output
"""
...
class RandomForestSpreadModel:
    """Random Forest model for NBA spread prediction."""
    
    def __init__(self, data_path):
        self.data_path = data_path
        self.df = None
        self.home_model = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42)
        self.away_model = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42)
    ...
    def backtest(self, start_date='2025-10-22', end_date='2025-11-16'):
        ...
        print(f"\n{'='*80}")
        print(f"🔬 BACKTESTING: RANDOM FOREST")
        print(f"{'='*80}")
        ...
        if len(results_df) > 0:
            ...
            # Save results
            output_file = 'data/random_forest_model_results.csv'
...
def main():
    """Run random forest model backtest."""
    model = RandomForestSpreadModel('data/NBA Training Set 25-26.csv')
    model.load_data()
    model.backtest()
