"""
NBA Spread Prediction - Linear Regression Model
Author: Orb Analytics (Liam Chaitin)
Purpose: Backtest linear regression model with detailed output
"""
...
class LinearSpreadModel:
    """Linear Regression model for NBA spread prediction."""
    ...
    def predict_game(self, game_row):
        ...
        # Linear regression outputs continuous value, clip to [0, 1]
        raw_pred = pipeline.predict(X_game)[0]
        model_fav_probability = np.clip(raw_pred, 0, 1)
    ...
    def backtest(self, start_date='2025-10-22', end_date='2025-11-16'):
        ...
        print(f"\n{'='*80}")
        print(f"🔬 BACKTESTING: LINEAR REGRESSION")
        print(f"{'='*80}")
        ...
        if len(results_df) > 0:
            ...
            # Save results
            output_file = 'data/linear_model_results.csv'
...
def main():
    """Run linear model backtest."""
    model = LinearSpreadModel('data/NBA Training Set 25-26.csv')
    model.load_data()
    model.backtest()
